...
// ```bash
// go run xds.go lds --proxytag httpbin-5766dd474b-2hlnx
// go run xds.go lds --proxytag httpbin
// ```
// Note If more than one pod match with the app label, one will be picked arbitrarily.
//
// To show only LDS with name 10.0.0.1_80
// ```bash
// go run xds.go lds --proxytag httpbin -r "10.0.0.1_80"
// ```
//
...
// ```bash
// go run xds.go lds --proxytag httpbin --kubeconfig path/to/kube/config
// ```
package main
